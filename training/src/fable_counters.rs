...
pub fn get_strategy(name: &str) -> Option<Strategy> {
    match name {
        "fable-splash-cluster" => Some(fable_splash_cluster::get_strategy(0)),
        "fable-wounded-finisher" => Some(fable_wounded_finisher::get_strategy(0)),
        "fable-healer-hunter" => Some(fable_healer_hunter::get_strategy(0)),
        "fable-econ-attrition-ball" => Some(fable_econ_attrition_ball::get_strategy(0)),
        "fable-cohesion-groups" => Some(fable_cohesion_groups::get_strategy(0)),
        "fable-cooldown-kiter" => Some(fable_cooldown_kiter::get_strategy(0)),
        _ => None,
    }
}
mod fable_splash_cluster {
    use crate::core::*;
    ...
    fn battle_strategy_for_opponent(
        ...
        let healer_goal = if support {
            ((battles + 1) / 2).max(battles.min(4)).min(10)
        } else if rush {
            (battles / 6).min(2)
        } else {
            (battles / 5).min(3)
        ...
        let guard = payload_guards(state, conf, 3);
        ...
        for bot_state in state.fleet_me.iter() {
            ...
            let rally = rally.or_else(|| medic_leash(state, conf, bot_state));
            let contester = guard & (1 << bot_state.id) != 0 && rally.is_none();
            bot_action.move_action =
            ...
            if bot_state.next_fire_tick() <= state.tick {
    ...
    fn payload_guards(state: &GameState, conf: &GameConfig, count: usize) -> u32 {
        ...
        candidates
            .into_iter()
            .take(count)
            .fold(0u32, |mask, (id, _)| mask | (1 << id))
    }

    fn medic_leash(state: &GameState, conf: &GameConfig, bot: &BotState) -> Option<Vec2> {
        if !state.fleet_other.iter().any(|enemy| {
            enemy.class() == BotClass::Battle && enemy.pos.dist(&bot.pos) <= conf.bot.blaster_range
        }) {
            return None;
        }
        let (medic, distance) = state
            .fleet_me
            .iter()
            .filter(|ally| ally.class() == BotClass::Healer)
            .filter_map(|ally| path_length(conf, bot.pos, ally.pos).map(|d| (ally, d)))
            .min_by(|(_, a), (_, b)| a.total_cmp(b))?;
        if distance <= conf.bot.base_heal_range * 1.5 {
            return None;
        }
        let meeting =
            medic.pos + (bot.pos - medic.pos).normalize_or_zero() * conf.bot.base_heal_range * 0.65;
                    ...
                    enemy.pos.dist(&payload) <= conf.bot.blaster_range + conf.payload.capture_radius
                        || enemy.pos.dist(&bot_state.pos)
                            <= conf.bot.blaster_range
                                * if enemy.health <= conf.bot.blaster_damage * 2.0 {
                                    1.4
                                } else {
                                    1.0
                                }
...
mod fable_healer_hunter {
    ...
    fn battle_strategy_for_opponent(
        ...
        let healer_goal = if support {
            ((battles as f32 / 2.5) as usize).min(9)
        } else if rush {
            (battles / 6).min(2)
        } else {
            (battles / 5).min(3)
            ...
            let enemy = state
                ...
                .filter(|enemy| {
                    enemy.pos.dist(&payload) <= conf.bot.blaster_range + conf.payload.capture_radius
                        || enemy.pos.dist(&bot_state.pos)
                            <= conf.bot.blaster_range
                                * if enemy.class() == BotClass::Healer {
                                    1.5
                                } else {
                                    1.0
                                }
                ...
                .min_by_key(|enemy| {
                    ...
                    (
                        ...
                        if exposed {
                            (enemy.health * 100.0) as u32
                                + u32::from(
                                    enemy.class() != BotClass::Healer
                                        && enemy.health > conf.bot.blaster_damage,
                                ) * 10000
                        } else {
                            0
                        },
...
mod fable_econ_attrition_ball {
    ...
    fn battle_strategy_for_opponent(
        ...
        // Field fighters while the opening is still being revealed. Match early rush pressure,
        // then use the normal economy against balanced and mining-heavy armies.
        let miner_goal = if state.tick < 8 || rush { 1 } else { 6 };
...
mod fable_cohesion_groups {
    ...
    fn battle_strategy_for_opponent(
        ...
        for bot_state in state.fleet_me.iter() {
            ...
            let rally = rally.or_else(|| {
                if guard & (1 << bot_state.id) != 0 {
                    None
                } else {
                    cohesion(state, conf, bot_state)
                }
            });
            let contester = guard & (1 << bot_state.id) != 0 && rally.is_none();
            bot_action.move_action =
    ...
    fn cohesion(state: &GameState, conf: &GameConfig, bot: &BotState) -> Option<Vec2> {
        // Stragglers join the nearest supported fighter group instead of trickling forward.
        let near = |b: &BotState| {
            state
                .fleet_me
                .iter()
                .filter(|a| {
                    a.id != b.id
                        && a.class() == BotClass::Battle
                        && a.pos.dist(&b.pos) <= conf.bot.base_heal_range
                })
                .count()
        };
        if near(bot) >= 2 {
        ...
        state
            .fleet_me
            .iter()
            .filter(|a| a.id != bot.id && a.class() == BotClass::Battle && near(a) >= 2)
            .min_by(|a, b| a.pos.dist_sq(&bot.pos).total_cmp(&b.pos.dist_sq(&bot.pos)))
            .map(|a| a.pos)
    }
...
mod fable_cooldown_kiter {
    ...
    fn battle_strategy_for_opponent(
        ...
        let healer_goal = if battles < 4 { 0 } else { (battles / 3).min(8) };
        if next_bot == BotClass::Battle && healers < healer_goal {
            next_bot = BotClass::Healer;
        }
        ...
        for bot_state in state.fleet_me.iter() {
            ...
            let goal = rally.unwrap_or_else(|| {
                ...
                // ponytail: shoot at max range, back off while cooling, re-approach when ready.
                let near = state
                    .fleet_other
                    .iter()
                    .filter(|b| {
                        b.class() == BotClass::Battle
                            && b.pos.dist(&bot_state.pos) < conf.bot.blaster_range * 0.75
                    })
                    .count();
                if !contester && near > 0 && bot_state.next_fire_tick() > state.tick + 10 {
                    let away = bot_state.pos
                        + (bot_state.pos - enemy.pos).normalize_or_zero()
                            * conf.bot.blaster_range
                            * 0.5;
                    if point_free(conf, away)
                        && obstacles(state, conf)
                            .iter()
                            .all(|&(p, r)| away.dist(&p) > r + conf.bot.radius)
                    {
                        return away;
                    }
                }
                firing_position(state, conf, bot_state.pos, enemy.pos, contester, true)
            });
            bot_action.move_action = spaced_move(state, conf, bot_state, goal, contester);
            // Match the engine's movement clamp before predicting this tick's firing origin.
            bot_action.move_action.sanitize();
