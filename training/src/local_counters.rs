...
pub fn get_strategy(name: &str) -> Option<Strategy> {
    match name {
        "local-distributed-medics" => Some(local_distributed_medics::get_strategy(0)),
        "local-supported-advance" => Some(local_supported_advance::get_strategy(0)),
        "local-combat-heavy" => Some(local_combat_heavy::get_strategy(0)),
        "local-strength-gate" => Some(local_strength_gate::get_strategy(0)),
        "local-finish-wounded" => Some(local_finish_wounded::get_strategy(0)),
        "local-three-guards" => Some(local_three_guards::get_strategy(0)),
        "local-stable-healing" => Some(local_stable_healing::get_strategy(0)),
        "local-cohesive-pressure" => Some(local_cohesive_pressure::get_strategy(0)),
        "v12-mirror" => Some(v12_mirror::get_strategy(0)),
        _ => None,
    }
}
mod local_distributed_medics {
    use crate::core::*;
            ...
            if bot_state.class() == BotClass::Healer {
                ...
                let follow = patient.or_else(|| {
                    state
                        ...
                        .min_by(|a, b| {
                            if !support {
                                return a.pos.dist_sq(&payload).total_cmp(&b.pos.dist_sq(&payload));
                            }
    ...
    fn battle_strategy_for_opponent(
        ...
        for bot_state in state.fleet_me.iter() {
            ...
            let rally = rally.or_else(|| {
                ...
                let goal = medic.pos
                    + (bot_state.pos - medic.pos).normalize_or_zero()
                        * conf.bot.base_heal_range
                        * 0.65;
                if point_free(conf, goal)
                    && obstacles(state, conf)
            ...
            let enemy = state
                ...
                .min_by_key(|enemy| {
                    ...
                    (
                        ...
                        // Attrition armies can heal or replace chip damage; finish their wounded.
                        // Keep nearest-target pressure against light economies, including healing formations.
                        if exposed && (observed.0 >= 6 || support) {
                            (enemy.health * 100.0) as u32
                        } else {
                            0
                        },
...
mod local_three_guards {
    ...
    fn battle_strategy_for_opponent(
        ...
        for bot_state in state.fleet_me.iter() {
            ...
            let extra_guard = support
                && state
                    .fleet_me
                    .iter()
                    .filter(|ally| ally.class() == BotClass::Battle)
                    .filter(|ally| {
                        (ally.pos.dist_sq(&payload), ally.id)
                            < (bot_state.pos.dist_sq(&payload), bot_state.id)
                    })
                    .count()
                    < 3;
            let contester = (guard == Some(bot_state.id) || extra_guard) && rally.is_none();
            bot_action.move_action =
                spaced_move(state, conf, bot_state, rally.unwrap_or(payload), contester);
